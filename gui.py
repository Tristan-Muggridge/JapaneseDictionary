...
class Application(tk.Frame):
    ...
    def submit(self, event):
        ...
        for entry in range(5):
            frame = tk.LabelFrame(self.submission_results, text=f"{entries[entry].kanji}・{entries[entry].hiragana}・{entries[entry].classes}", 
                        pady=1,padx=5)
            ...
            for kanji in entries[entry].filtered_kanji:
                kanji_button = tk.Button(frame, text=kanji, command=partial(self.kanji_button_clicked, kanji))
    ...
    def kanji_button_clicked(self, kanji):
        ...
        self.submission_results.destroy()
        for entry in range(5):
            self.submission_results = tk.LabelFrame(self.kanji_results, text=kanji, 
                        pady=1,padx=5)
